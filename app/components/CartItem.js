...
export default function CartItem({ item, checked, onCheck, onClick }) {
  ...
  return (
    <div
      className={`flex gap-4 bg-white rounded-2xl p-4 border transition-colors ${checked ? "border-black" : "border-gray-100"} shadow-sm`}>
      {/* 체크박스 */}
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-2">
          <p className="text-sm font-bold text-black leading-snug line-clamp-2">
            {item.name}
          </p>
          <button
